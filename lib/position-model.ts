...
export function buildPositions(trades: Trade[]): Position[] {
  ...
  for (const [key, groupTrades] of map) {
    ...
    //
    // Note: buy fills that have a resting sell order attached carry
    // remaining_count=-1 (the sell-order sentinel) but are still genuine
    // purchased contracts — they must be included in contractsBought.
    // Contracts actually sold are captured exclusively via outcome="sold" fills.
    const soldFills = fills.filter((t) => t.outcome === "sold");

    const contractsBought = fills.reduce(
      (s, t) => s + getContractsForFill(t),
      0,
    );
